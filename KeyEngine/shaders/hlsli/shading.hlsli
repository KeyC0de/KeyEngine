...
{
	return 1.0f / ( attConstant + attLinear * fragmentToLightDist
		+ attQuadratic * (fragmentToLightDist * fragmentToLightDist) );
}

float3 calculateLightDiffuseContribution( uniform float3 diffuseColor,
	uniform float intensity,
	const in float att,
	const in float3 viewSpaceFragmentToLightDir,
	const in float3 viewSpaceNormal )
{
	return att * diffuseColor * intensity * max( 0.0f,
		dot( viewSpaceFragmentToLightDir,
			viewSpaceNormal ) );
}
...
float3 calculateLightSpecularContribution( const in float3 lightColor,
	...
	const in float att )
{
	const float3 specularReflection = normalize( 2.0f * viewSpaceNormal * dot( viewSpaceFragmentToLightDir,
		viewSpaceNormal ) - viewSpaceFragmentToLightDir );
	...
	return lightColor * specularColor * intensity * att * pow( max( 0.0f, dot( -specularReflection,
			viewSpacePosNormalized ) ),
		fragSpecularGloss );
}

float3 calculateNormalMapNormal( const in float3 tangentViewSpace,
	const in float3 bitangentViewSpace,
	const in float3 normalViewSpace,
	const in float2 tc,
	uniform Texture2D normalMap,
	uniform SamplerState sampl )
{
	// build the TBN rotation matrix
	const float3x3 tbn = float3x3( tangentViewSpace,
		bitangentViewSpace,
		normalViewSpace);
	// sample normal map
	const float3 normalMapNormal = normalMap.Sample( sampl,
		tc ).xyz;
	...
	return normalize( mul( tangentSpaceNormal,
		tbn ) );
}