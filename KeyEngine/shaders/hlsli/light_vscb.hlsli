...
/// \brief cb_lightMatrix performs a coordinate transformation whose origin is at the Light's origin location
cbuffer LightVSCB : register(b2)
{
	matrix cb_lightMatrix[MAX_LIGHTS];
};

/// \brief Directional and Spotlight Shadows: The pixel shader samples the shadow map using the transformed light-space position.
...
/// \brief Point Light Shadows: The PS calculates the direction to the point light and performs a cubemap lookup using the world-space position.
