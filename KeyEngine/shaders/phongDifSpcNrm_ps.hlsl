...
PSOut main( PSIn input )
{
	...
	if ( shadowLevel != 0.0f )
	{
		...
		if ( bNormalMap )
		{
			const float3 mappedNormal = calculateNormalMapNormal( normalize( input.tangentViewSpace ),
				normalize( input.bitangentViewSpace ),
				input.fragNormalViewSpace,
				input.tc,
				normTex,
				sampl );
			input.fragNormalViewSpace = lerp( input.fragNormalViewSpace,
				mappedNormal,
				normalMapStrength );
		}
		...
		if ( bSpecularMap )
		{
			const float4 specMapSample = specTex.Sample( sampl,
				input.tc );
