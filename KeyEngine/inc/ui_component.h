...
private:
	...
	static inline Component *s_root = nullptr;	// set when the first Component is created, see remarks @ Component ctor
	static inline std::vector<Component*> s_top_most_components;	// refers to components in the ui hierarchy that are top most
	// world space components collision shape is always relevant to a Mesh; but they are still rendered in screen space and as such always facing the camera
	// eg. "character health bar" is considered world space component
	static inline std::vector<Component*> s_world_space_components;	// refers to components in the ui hierarchy that are world space
	static inline float s_scale = 1.0f;
	static inline float s_last_dt = 0.0f;
	static inline Component* s_last_hover = nullptr;	// the component, excluding root, that was last hovered - updated on on_hover_off()
	static inline Component* s_current_hover = nullptr;	// the component, excluding root, that is currently being hovered - updated on evaluate_current_hover()
	static inline Point s_last_input_pos{0, 0};			// set on update() by root
	static inline float s_lerpBetweenFrames = 0.0f;		// set on update() by root
	static inline int s_current_tick = 0;				// set on update() by root
...
private:
	...
	float m_depth;				// Components with smaller depth numbers will render on top
	bool m_is_visible = true;	// root component is an imaginary component and is always invisible
	bool m_update_when_not_visible = false;
	bool m_is_topmost_object = false;
	bool m_can_move = true;
	bool m_is_tooltip = false;	// a tooltip Component will be created as a child of this one
	Aspect_Ratio_Locked_Behavior m_aspect_ratio_locked_behavior = Aspect_Ratio_Locked_Behavior::Aspect_Ratio_None;
	Mesh* m_mesh = nullptr;
	Component* m_parent;					// the parent Component of the root is nullptr
	std::unique_ptr<Component> m_tooltip;	// tooltips do not exist in the main tree hierarchy, but are owned and managed by the individual components they exist in
	std::vector<std::unique_ptr<Component>> m_children;	// this component's children, all events are passed down the hierarchy to children
...
public:
	//	\function	create_component	||	\date	2024/05/17 19:14
...
private:
	...
	//	\function	render_world_space_components	||	\date	2024/05/17 15:37
	...
	//	\function	render_top_most_components	||	\date	2024/05/16 19:48
...
public:
	...
	//	\function	update	||	\date	2024/05/16 20:22
	...
	//	\function	reevaluate_current_hover	||	\date	2024/05/16 19:01
	...
	//	\function	has_current_hover	||	\date	2024/05/16 20:41
	...
	//	\function	find	||	\date	2024/05/17 19:04
	...
	//	\function	divorce_child	||	\date	2024/05/16 16:15
	//	\brief	removes the specified child from our child list
	//			returns the index of where the child was in the child list
	...
	//	\function	get_user_property_value	||	\date	2024/05/15 21:16
	...
	bool is_parent_of( const Component &comp ) const noexcept;	// checks whether supplied component is a parent, or ancenstor of this
	const Component* get_parent() const noexcept;
	Component* get_parent();
	const Component* get_parent( const std::string &name ) const noexcept;	// retrieve a named parent component
	Component* get_parent( const std::string &name );
	const Component* get_child( const std::string &name ) const noexcept;	// retrieve child with specified name
	Component* get_child( const std::string &name );
	const Component* get_child( const unsigned index ) const noexcept;	// retrieve child at specified index
	...
	int get_width() const noexcept;		// retrieve the width of the current state
	int get_height() const noexcept;	// retrieve the height of the current state
