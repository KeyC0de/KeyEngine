	...
	m_path{filepath}
{
	...
	HRESULT hres = D3DReadFileToBlob( util::s2ws( "int/x64/shaders/Debug/" + filepath ).c_str(),
		&m_pVsBlob );
#else
	HRESULT hres = D3DReadFileToBlob( util::s2ws( "int/x64/shaders/Release/" + filepath ).c_str(),
		&m_pVsBlob );
#endif
	ASSERT_HRES_IF_FAILED_MSG( filepath );
	hres = getDevice( gph )->CreateVertexShader( m_pVsBlob->GetBufferPointer(),
