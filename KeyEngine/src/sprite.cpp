	...
	Graphics &gph )
{
	...
	hres = gph.renderTarget2d()->CreateBitmapFromWicBitmap( m_pWicConverter.Get(),
		nullptr,
		&m_pBitmap );
	ASSERT_HRES_IF_FAILED;
}
...
void Sprite::render( Graphics &gph,
	...
	const float alpha )
{
	gph.renderTarget2d()->DrawBitmap( m_pBitmap.Get(),
		destRect,
		alpha,
...
const float Sprite::getWidth() const noexcept
{
	return m_pBitmap->GetSize().width;
}

const float Sprite::getHeight() const noexcept
{
	...
	const float alpha )
{
	...
	D2D1_RECT_F srcRect = D2D1::RectF( (float) srcRectX,
		(float) srcRectY,
		(float) srcRectX + m_spriteWidth,
		(float) srcRectY + m_spriteHeight );
	D2D1_RECT_F destRect = D2D1::RectF( (float) x,
		(float) y,
		(float) x + m_spriteWidth,
		(float) y + m_spriteHeight );
