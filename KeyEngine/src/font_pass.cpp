...
namespace
{
	auto &g_setMan = SettingsManager::instance();
}
	...
	IBindablePass{name}
{
	...
	m_pFpsSpriteFont = std::make_unique<dx::SpriteFont>( getDevice( gph ),
		fpsFontRelativePath.c_str() );
...
void FontPass::updateAndRenderFpsTimer()
{
	...
	m_pFpsSpriteFont->DrawString(m_pSpriteBatch.get(),
		fpsText.c_str(),
