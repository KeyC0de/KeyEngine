	...
	std::optional<unsigned> face )
{
	...
	HRESULT hres = getDevice( gph )->CreateDepthStencilView( pTexture, &dsvDesc, &m_pD3dDsv );
	...
	m_height(height)
{
	...
	hres = getDevice( gph )->CreateDepthStencilView( pTexture.Get(), &dsViewDesc, &m_pD3dDsv );
	ASSERT_HRES_IF_FAILED;
}
...
void IDepthStencilView::bindRenderSurface( Graphics &gph ) cond_noex
{
	...
	getDeviceContext( gph )->OMSetRenderTargets( 0u, nullptr, m_pD3dDsv.Get() );
	...
	IRenderSurface *rt ) cond_noex
{
	...
	bindRenderSurface( gph, static_cast<IRenderTargetView*>( rt ) );
}

void IDepthStencilView::bindRenderSurface( Graphics &gph,
	IRenderTargetView *rt ) cond_noex
{
	rt->bindRenderSurface( gph, this );
}

void IDepthStencilView::clear( Graphics &gph,
	const std::array<float, 4> &unused ) cond_noex
{
	getDeviceContext( gph )->ClearDepthStencilView( m_pD3dDsv.Get(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0ui8 );
	DXGI_GET_QUEUE_INFO( gph );
}
...
void IDepthStencilView::clean( Graphics &gph ) cond_noex
{
	...
	getDeviceContext( gph )->OMSetRenderTargets( 0u, nullptr, nullptr );
...
std::pair<Microsoft::WRL::ComPtr<ID3D11Texture2D>, D3D11_TEXTURE2D_DESC> IDepthStencilView::createStagingTexture( Graphics &gph ) const
{
	...
	{
		getDeviceContext( gph )->CopySubresourceRegion( pStagingTex.Get(), 0u, 0u, 0u, 0u, pDsTex.Get(), dsvDesc.Texture2DArray.FirstArraySlice, nullptr );
		DXGI_GET_QUEUE_INFO( gph );
	}
	else
	{
		getDeviceContext( gph )->CopyResource( pStagingTex.Get(), pDsTex.Get() );
		DXGI_GET_QUEUE_INFO( gph );
	}
	...
	bool bLinearize /*= true*/ ) const
{
	...
	for ( unsigned y = 0u; y < height; ++y )
	{
		...
		for ( unsigned int x = 0; x < width; ++x )
		{
			...
			{
				getDeviceContext( gph )->Unmap( pStagingTex.Get(), 0u );
...
DepthStencilShaderInput::DepthStencilShaderInput( Graphics &gph,
	...
	const DepthStencilViewMode dsvMode )
	:
	DepthStencilShaderInput(gph, gph.getClientWidth(), gph.getClientHeight(), slot, dsvMode)
{
...
DepthStencilShaderInput::DepthStencilShaderInput( Graphics &gph,
	...
	:
	IDepthStencilView(gph, width, height, true, dsvMode),
	m_slot(slot)
{
	...
	HRESULT hres = getDevice( gph )->CreateShaderResourceView( pRes.Get(), &srvDesc, &m_pD3dSrv );
	ASSERT_HRES_IF_FAILED;
}
...
DepthStencilOutput::DepthStencilOutput( Graphics &gph,
	...
	const unsigned height )
	:
	IDepthStencilView(gph, width, height, false, DepthStencilViewMode::DefaultDS)
{
...
DepthStencilOutput::DepthStencilOutput( Graphics &gph,
	...
	std::optional<unsigned> face /*= {}*/ )
	:
	IDepthStencilView{gph, pTexture, dsvMode, face}
{
