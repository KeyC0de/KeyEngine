	...
	unsigned wicFlags )
{
	...
	HRESULT hres = dx::LoadFromWICFile( util::s2ws( name ).c_str(),
		static_cast<dx::WIC_FLAGS>( wicFlags ),
		nullptr,
		scratchImg );
	if ( FAILED( hres ) )
	{
...
Bitmap::Texel Bitmap::vectorToColor( const dx::XMVECTOR& v ) noexcept
{
	...
	nOut = dx::XMVectorMultiply( nOut,
		all255 );
	dx::XMFLOAT3 floats;
	dx::XMStoreFloat3( &floats,
		nOut );
	return {(unsigned char)round( floats.x ),
...
const std::string Bitmap::BitmapException::getType() const noexcept
{
