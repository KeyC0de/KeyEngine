	...
	m_file{filename, std::ios::out | std::ios::in | std::ios::binary}
{
	...
	m_file.read( reinterpret_cast<unsigned char*>( &m_bmpFileHeader ),
		sizeof m_bmpFileHeader );
	// m_bmpFileHeader.bfType		// the file type; must be "BM".
	// m_bmpFileHeader.bfSize		//	size of the bitmap file

	m_file.read( reinterpret_cast<unsigned char*>( &m_bmpInfoHeader ),
...
bool BmpLoader::applyPerlinNoise( const std::string &filename /*= ""*/ )
{
	...
	auto perlin = [] ( const double x, const double y ) -> double
	{
		...
		static KeyRandom r;
		static auto moreRandom = r.getRandomIntInRange( 64,
			256 );
		static auto lessRandom = r.getRandomIntInRange( 8,
			32 );

		// wood like structure
		double n = lessRandom * perlin.noise( x,
			y,
			moreRandom );
		return ( n - floor( n ) ) * 255.0;
	};

	std::function<double(double, double)> perlinNoiseFilter = std::bind( perlin,
		std::placeholders::_1,
		std::placeholders::_2 );
	...
	//std::function<double(double, double)> gaussianFilter = std::bind( util::gaussian2d<double>,
		//std::placeholders::_1,
		//std::placeholders::_2,
		//1.0 );
