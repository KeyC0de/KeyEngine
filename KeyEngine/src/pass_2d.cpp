	...
	IBindablePass{name}
{
	...
	addPassBindable( std::make_shared<DepthStencilState>( gph, DepthStencilState::DepthOffStencilOff ) );

	auto pVs = VertexShader::fetch( gph, "flat2d_vs.cso" );
	...
	addPassBindable( InputLayout::fetch( gph, vb.getLayout(), *pVs ) );

	addPassBindable( std::make_shared<VertexBuffer>( gph, vb ) );

	addPassBindable( std::move( pVs ) );

	std::vector<unsigned> indices = {0, 1, 2, 2, 3, 0};
	addPassBindable( std::make_shared<IndexBuffer>( gph, indices ) );

	addPassBindable( std::make_shared<PixelShader>( gph, "flat2d_ps.cso" ) );

	addPassBindable( std::make_shared<Texture>( gph, gph.getClientWidth(), gph.getClientHeight(), 0u ) );
	...
	addPassBindable( TextureSamplerState::fetch( gph, TextureSamplerState::TextureSamplerMode::DefaultTS, TextureSamplerState::FilterMode::Point, TextureSamplerState::AddressMode::Clamp ) );

	//m_sprite = std::make_unique<Sprite>( LR"(assets\textures\ba2_icon.png)", gph );
	//m_player = std::make_unique<SpriteSheet>( LR"(assets\textures\player.png)", gph, 32, 32 );
...
/*	for ( int i = 0; i < 10; ++i )
	{
		gph.drawCircle( m_randomness.rand() % 800, m_randomness.rand() % 600, m_randomness.rand() % 100, D2D1::ColorF{m_randomness.getRandomFloat(), m_randomness.getRandomFloat(), m_randomness.getRandomFloat(), m_randomness.getRandomFloat() / 1.75 + 0.25f},
	...
	gph.drawText( L"KeyC0de", rect, D2D1::ColorF{0, 111 / 256.f, 0} );

	// image
	const auto ba2Rect = D2D1_RECT_F{0, 0, m_sprite->getWidth(), m_sprite->getWidth()};
	m_sprite->render( gph, ba2Rect, ba2Rect, .6f );

	m_player->render( gph, m_spriteOrder[m_currentSpriteIndex], 300, 100 );
*/
	bind( gph );
	gph.drawIndexed( 6u );
}
