	...
	int screenY )
{
	...
	dx::XMStoreFloat4( &coordsView,
		convertToViewSpace( gph, coordsClip ) );

	// convert to world space
	dx::XMStoreFloat3( &m_rayDirectionWorldSpace,
		convertToWorldSpace( gph, coordsView ) );
	...
	const dx::XMFLOAT2 &coordsNdc )
{
	...
	dx::XMFLOAT4 row1f;
	dx::XMStoreFloat4( &row1f,
		row1 );
	float zoomX = row1f.x;

	dx::XMVECTOR row2 = projectionMatrix.r[1];
	dx::XMFLOAT4 row2f;
	dx::XMStoreFloat4( &row2f,
		row2 );
	float zoomY = row2f.y;
