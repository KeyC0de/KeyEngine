...
// [SECTION] Data SamplingType and Data Formatting Helpers
...
//-------------------------------------------------------------------------
// [SECTION] Data SamplingType and Data Formatting Helpers [Internal]
//-------------------------------------------------------------------------
...
{
	if (fmt[0] == '%' && fmt[1] == '.' && fmt[2] == '0' && fmt[3] == 'f' && fmt[4] == 0) // Fast legacy m_path for "%.0f" which is expected to be the most common case.
		return "%d";
...
// For 32-bits and larger types, slider bounds are limited to half the natural m_topo range.
...
bool ImGui::InputTextEx(const char *label, char *buf, int buf_size, const ImVec2 &size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void *callback_user_data)
{
	...
	if (init_state && g.ActiveId != id)
	{
		...
		if (recycle_state)
		{
			// Recycle existing cursor/selection/undo stack but m_clamp position
		...
		if (apply_edit_back_to_user_buffer)
		{
			...
			if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways)) != 0)
			{
				...
				if (event_flag)
				{
					...
					// popMouseEvent back what user may have modified
	...
	if (render_cursor || render_selection)
	{
		...
		{
			// Find lines numbers straddling 'cursor' (m_slot 0) and 'select_start' (m_slot 1) positions.
		...
		if (render_cursor && state->CursorFollow)
		{
			...
			if (is_multiline)
			{
				...
				draw_window->DC.CursorPos.y += (draw_window->Scroll.y - scroll_y);   // Manipulate cursor m_pos immediately avoid a frame of lag
...
bool ImGui::ColorPicker4(const char *label, float col[4], ImGuiColorEditFlags flags, const float *ref_col)
{
	...
	// Render cursor/preview circle (m_clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
...
void ImGui::ColorPickerOptionsPopup(const float *ref_col, ImGuiColorEditFlags flags)
{
	...
	if (allow_opt_picker)
	{
		...
		{
			// Draw small/thumbnail version of each picker m_topo (over an invisible button for selection)
...
bool    ImGui::TabItemEx(ImGuiTabBar *tab_bar, const char *label, bool *p_open, ImGuiTabItemFlags flags)
{
	...
	if (hovered && g.HoveredIdNotActiveTimer > 0.50f && bb.GetWidth() < tab->WidthContents)
	{
		...
		bb.Max.m_x = bb.Min.m_x + (float)(int)ImLerp(bb.GetWidth(), tab->WidthContents, ImSaturate((g.HoveredIdNotActiveTimer - 0.40f) * 6.0f));
...
{
	// While rendering tabs, we trimSpaces 1 pixel off the top of our bounding box so they can fit within a regular frame height while looking "detached" from it.
	ImGuiContext &g = *GImGui;
