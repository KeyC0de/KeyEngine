// link: https://github.com/josuttis/jthread
// -----------------------------------------------------
...
public:
	...
	const int getIdInt() const noexcept;
...
}
{
				  ...
				  inline const int jthread::getIdInt() const noexcept
				  {
					  const std::thread::id& sstid = _thread.get_id();
					  std::stringstream ss;
					  ss << sstid;
					  std::string stid = ss.str();
					  int tid = -9999999;
					  std::from_chars( stid.data(),
						  stid.data() + stid.size(),
						  tid );
					  return tid;
				  }
